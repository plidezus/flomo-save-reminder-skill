...
- **先问你想要多主动**：第一次使用会先让你选提醒方式，是只在你说“记一下”时出现，还是你主动让它扫描对话，或只在本轮轻提醒。
...
第一次用的时候，skill 会先问你希望它多主动：

1. 只在我说“保存/记一下”时使用
2. 我让你扫描当前对话时，帮我挑值得保存的内容
3. 本轮对话里，看到特别值得留的内容可以轻提醒

选完以后，它再通过 flomo MCP 的 memory 和 memo 样本生成你的本地画像。生成完把 `profile_status` 改成 `generated`，把你的选择写进 `reminder_mode`，下次直接读。
