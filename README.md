...
- **flomo-shared**：共享的 profile 与偏好。让另外两个 skill 用同一份「你」。
...
- **你的写法只学一次**：两个 skill 共用同一份 profile，不重复学，也不互相污染。
...
`flomo-shared/SKILL.md` 管 profile 怎么生成、什么时候刷新；`profile.md` 存你的写作画像和你授权过的偏好。保存提醒和昨日回声不各自生成画像，只读取共享 profile。

如果你之前装过旧版（`flomo-save-reminder/user-style.md` 或 `state.md` 还在），新版会在首次需要 profile 时提示迁移。画像可以迁移，授权过的偏好可以迁移，临时运行状态不迁移。
